...
Furthermore, there are only two unique combinations between `rv_lanes_stack_type` and `hi_flow_rv_lanes_stack_type`: either both "None" or both "HF/RV". It seems extremely likely to me that "HF/RV" stands for "Hi(gh) Flow / Recreational Vehicle". If that is true and it is also true that "N/A" and "None" mean the absence of the attribute (and not the absence of the data), then it is necessarily true that
...
lapply(c("inside_sales", "food_service", "diesel", "unleaded"),
  ...
  arrange(target, desc(nchar(sig)), sig, -abs(Estimate))
...
lapply(c("inside_sales", "food_service", "diesel", "unleaded"),
  ...
  list_rbind() %>%
  filter(!grepl("Intercept", var)) %>%
  arrange(desc(nchar(sig)), sig, -abs(Estimate)) 
```
